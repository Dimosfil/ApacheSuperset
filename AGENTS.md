...
Apache Superset demo workspace for preparing a full-stack BI customization
demo. The expected surface is a self-contained Superset-based environment with
Docker, PostgreSQL-compatible demo data, Python/Flask-side configuration or
extensions, and React/TypeScript frontend or visualization work as needed.
...
```powershell
# Define once the demo stack is selected.
```

Run:

```powershell
# Define once Docker/Superset entrypoints are added.
```

Test:

```powershell
# Define once project code and checks exist.
```

Build:

```powershell
# Define once frontend or packaging artifacts exist.
```

Inspect logs:

```powershell
# Define once services are added.
```

## Working Areas

- Source: project root until the demo layout is created
- Tests: define with the first implementation slice
- Tools: `tools/`
